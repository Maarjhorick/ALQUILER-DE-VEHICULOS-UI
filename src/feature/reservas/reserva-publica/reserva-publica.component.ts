...
export class ReservaPublicaComponent {
  ...
  readonly vehiculosDisponibles = computed(() =>
    this.vehiculoService.vehiculos().filter((vehiculo) => vehiculo.estado === 'DISPONIBLE')
  );
  ...
  readonly vehiculoSeleccionado = computed(() =>
    this.vehiculosDisponibles().find((vehiculo) => vehiculo.id === Number(this.reserva.vehiculoId))
  );
